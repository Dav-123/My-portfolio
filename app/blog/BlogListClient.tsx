...
export default function BlogListClient({ posts }: { posts: PostMeta[] }) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-background via-background to-secondary/20 px-4 sm:px-6 py-24 sm:py-32">
      <div className="fixed top-0 right-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] rounded-full bg-primary/8 blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto relative z-10" ref={ref}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-14">
          <p className="text-primary font-semibold text-sm tracking-widest uppercase mb-4">Writing</p>
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-bold text-foreground mb-5">The Blog</h1>
          <p className="text-muted-foreground text-sm sm:text-lg max-w-xl">Lessons from building real products, thoughts on African tech, and the philosophy behind writing code that actually matters.</p>
        </motion.div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-muted-foreground text-sm sm:text-base">No posts in this category yet. Check back soon!</div>
        ) : (
